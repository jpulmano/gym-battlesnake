...
env_new = wrap_function(gamelib, 'env_new', ctypes.c_void_p, [ctypes.c_uint,ctypes.c_uint,ctypes.c_uint,ctypes.c_uint])
...
class ParallelBattlesnakeEnv(VecEnv):
    ...
    def __init__(self, n_threads=4, n_envs=16, n_opponents=7, opponent=None, device='cpu', fixed_orientation=0):
...
class BattlesnakeEnv(VecEnv):
    """Multi-Threaded Multi-Agent Snake Environment"""
    def __init__(self, n_threads=4, n_envs=16, opponents=[], device=torch.device('cpu'), fixed_orientation=0):
        # Define action and observation space
        self.action_space = spaces.Discrete(4)
        self.observation_space = spaces.Box(low=0,high=255, shape=(NUM_LAYERS, LAYER_WIDTH, LAYER_HEIGHT), dtype=np.uint8)
